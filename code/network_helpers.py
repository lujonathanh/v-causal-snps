...
def load_hg_ensg():
    ensg_hg_file = data_dir +  "gencode.v22.gene_id_to_gene_name.json"
...
def load_genes(filename, header=False, verbose=True):
    genes = set()
...
def make_cause_col(df, cause_col, effect_col, col_name = "Cause-Effect"):
    ...
    df[col_name] = ["-".join(x) for x in zip(df[cause_col],df[effect_col])]

    return df

def make_type_col(df, name, col_name=None):
    """
    :param df:
    :param name:
    :param col_name:
...
def annotate_cols_summary(df, cols, col_name = "Type"):
    """Assumes there is a 0 or non-zero in each of the cols. Summarizes them one-by-one"""
...
def matr_to_net(matr_df, name, edge_name=None, cause_effect_col = "Cause-Effect", colnames=None, make_pair=True):
    """
    :param matr_df: rows and columns are the genes
    :param name: Suffix to append to "Cause" and "effect"
    :param edge_name: Column name of the matrix values
    ...
    """
    if colnames == None:
        colnames = ["Cause:" + name, "Effect:" + name]

    if edge_name == None:
        edge_name = "Weight:" + name
...
def overlay_dfs(old_df, over_df, key = "Pair", over_cols=[], genecols=genecols, how='outer', fill_empty=False):
    """
    ...
    if fill_empty:
        df = fill_empty_genecols(df, genecols, key)
